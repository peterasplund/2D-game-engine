...
void Renderer::renderSprite(Sprite *sprite, v2i position, bool useOffset) {
  ...
  SDL_RenderCopyEx(_renderer, sprite->texture, &sr, &dr, 0, 0,
                   sprite->textureFlip);
}

void Renderer::renderTexture(SDL_Texture *texture, Rect *sr, Rect *dr,
                             SDL_RendererFlip textureFlip, bool useOffset) {
...
void Renderer::renderRectTexture(Rect rectangle, SDL_Texture *texture,
                                 int tileWidth) {
  ...
  drawCorner(tileWidth, texture, rectangle.x + rectangle.w - tileWidth,
             rectangle.y, SDL_FLIP_HORIZONTAL);
  drawCorner(tileWidth, texture, rectangle.x,
             rectangle.y + rectangle.h - tileWidth, SDL_FLIP_VERTICAL);
  drawCorner(tileWidth, texture, rectangle.x + rectangle.w - tileWidth,
             rectangle.y + rectangle.h - tileWidth,
...
void Renderer::drawSide(SIDE side, int tileWidth, Rect rect,
                        SDL_Texture *texture) {
  SDL_Rect sr;
  SDL_Rect dr;
  int framePadding = (tileWidth / 2);
...
void Renderer::drawCorner(int tileWidth, SDL_Texture *texture, const int x,
                          const int y, const SDL_RendererFlip flip) {
