  ...
  for (auto level : world->levels) {
    ...
    for (int x = 0; x < levelWidth; x++) {
      for (int y = 0; y < levelHeight; y++) {
        ...
        if (level.neighbours[NeighBourDirection::N].size() == 0) {
          cell.north = BorderValue::Wall;
        }
        if (level.neighbours[NeighBourDirection::E].size() == 0) {
          cell.east = BorderValue::Wall;
        }
        if (level.neighbours[NeighBourDirection::S].size() == 0) {
          cell.south = BorderValue::Wall;
        }
        if (level.neighbours[NeighBourDirection::W].size() == 0) {
          cell.west = BorderValue::Wall;
        }
...
void MapHud::drawCell(MapCell cell, Marker marker, v2i playerCellPos) {
  ...
  if (gameState
          .visited[(cell.position.y * _mapSizeInCells.x) + cell.position.x]) {
    switch (cell.type) {
  ...
  case Marker::Player:
    ...
    Rect playerMarker = {
        ...
        ((innerRect.y + (MAP_HUD_CELL_HEIGHT - 2) / 2) -
         (playerMarkerSize / 2)),
